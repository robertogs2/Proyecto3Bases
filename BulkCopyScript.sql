...
INSERT INTO Employee
 (fk_idPerson, fk_idPark, fk_idProfession, fk_idAssignedArea, fk_idSpeciality, Salary)
SELECT
...
INSERT INTO Accommodation
 (fk_idPark, Category, Capacity, BasePrice)
SELECT
 ...
 xData.value('BasePrice[1]', 'int') BasePrice
...
INSERT INTO Area
 ("Name", Size, fk_idPark)
SELECT
 xData.value('Name[1]', 'VARCHAR(50)') "Name",
 xData.value('Size[1]', 'VARCHAR(50)') Size,
 xData.value('ParkId[1]', 'VARCHAR(50)') fk_idPark
FROM @fileData.nodes('/DB/Areas/Area') AS x(xData)

INSERT INTO TourType
 ("Description")
SELECT
 xData.value('Description[1]', 'VARCHAR(50)') "Description"
FROM @fileData.nodes('/DB/TourTypes/TourType') AS x(xData)

-- insert the xml data into our Excursion table ("Name", "Description", Price, Capacity, fk_idVehicle)
INSERT INTO Tour
 ("Name", "Description", Price, Capacity, fk_idTourType, fk_idArea)
SELECT
 xData.value('Name[1]', 'VARCHAR(15)') "Name", -- 'xData' is our xml content alias
 xData.value('Description[1]', 'VARCHAR(50)') "Description",
 xData.value('Price[1]', 'int') Price,
 xData.value('Capacity[1]', 'int') Capacity,
 xData.value('TourTypeId[1]', 'int') fk_idTourType,
 xData.value('idArea[1]', 'int') fk_idArea
